...
export default {
	async fetch(request, env, ctx) {
		...
		if (env.PROXYIP) {
			...
			启用反代兜底 = false;
		} else 反代IP = (`${request.cf?.colo || 'auto'}.${查杀特征码}.CmLiUsSsS.nEt`).toLowerCase();
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						...
						if (订阅类型 === 'mixed') {
							...
							const ECHLINK参数 = 生成ECHLINK参数(config_JSON, url.host);
							const isLoonOrSurge = ua.includes('loon') || ua.includes('surge');
							const { type: 传输协议, 路径字段名, 域名字段名 } = 获取传输协议配置(config_JSON);
							const ALPNLINK参数 = config_JSON.协议类型 !== 'ss' && 传输协议 === 'ws' ? '&alpn=h2%2Chttp%2F1.1' : '';
							订阅内容 = 其他节点LINK + 完整优选IP.map(原始地址 => {
								...
								if (协议类型 === 'ss' && !作为优选订阅生成器) {
									...
									const ECH域名节点 = config_JSON.ECH ? 替换星号为随机字符(config_JSON.HOSTS?.[0] || url.host) : null;
									const 链接域名 = ECH域名节点 || 'example.com';
									const 固定订阅连接目标列表 = 解析订阅连接目标列表(config_JSON.订阅连接目标, 节点端口);
									const 订阅连接目标列表 = 固定订阅连接目标列表.length > 0 ? 固定订阅连接目标列表.concat([null]) : [null];
									return 订阅连接目标列表.map(固定订阅连接目标 => {
										const 链接节点地址 = 固定订阅连接目标?.地址 || ECH域名节点 || 节点地址;
										const 链接节点端口 = 固定订阅连接目标?.端口 || (ECH域名节点 ? 443 : 节点端口);
										const 多边缘节点备注 = 固定订阅连接目标列表.length > 0 ? `${节点备注}-${固定订阅连接目标 ? 链接节点地址 : 'domain'}` : 节点备注;
										return `${协议类型}://00000000-0000-4000-8000-000000000000@${链接节点地址}:${链接节点端口}?security=tls&type=${传输协议 + ECHLINK参数}&${域名字段名}=${链接域名}&fp=${config_JSON.Fingerprint}${ALPNLINK参数}&sni=${链接域名}&${路径字段名}=${encodeURIComponent(传输路径参数值) + TLS分片参数}&encryption=none#${encodeURIComponent(多边缘节点备注)}`;
									}).join('\n');
								}
...
function 生成ECHLINK参数(config_JSON, hostname) {
	if (!config_JSON?.ECH) return '';

	const echHostname = hostname || config_JSON.HOSTS?.[0] || config_JSON.HOST;
	return `&ech=${encodeURIComponent(`${echHostname}+udp://223.5.5.5:53`)}`;
}

function 解析订阅连接目标(订阅连接目标, 默认端口 = '443') {
	if (!订阅连接目标) return null;
	const 原始目标 = String(订阅连接目标).trim();
	if (!原始目标) return null;

	const 清理目标 = 原始目标.replace(/^https?:\/\//i, '').split('/')[0].split('?')[0].split('#')[0].trim();
	const 匹配结果 = 清理目标.match(/^(\[[\da-fA-F:]+\]|[\d.]+|[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?)*)(?::(\d+))?$/);
	if (!匹配结果) return null;

	return {
		地址: 匹配结果[1],
		端口: 匹配结果[2] || String(默认端口 || '443'),
	};
}

function 选择订阅连接目标(订阅连接目标, 默认端口 = '443', 分散键 = '') {
	if (!订阅连接目标) return null;
	const 可用目标 = 解析订阅连接目标列表(订阅连接目标, 默认端口);
	if (可用目标.length === 0) return null;
	if (可用目标.length === 1) return 可用目标[0];

	let hash = 0;
	for (const 字符 of String(分散键 || '')) hash = ((hash << 5) - hash + 字符.charCodeAt(0)) | 0;
	return 可用目标[Math.abs(hash) % 可用目标.length];
}

function 解析订阅连接目标列表(订阅连接目标, 默认端口 = '443') {
	if (!订阅连接目标) return [];
	const 目标列表 = Array.isArray(订阅连接目标) ? 订阅连接目标 : [订阅连接目标];
	const 已见 = new Set();
	return 目标列表.map(目标 => 解析订阅连接目标(目标, 默认端口)).filter(目标 => {
		if (!目标) return false;
		const key = `${目标.地址}:${目标.端口}`;
...
async function 读取订阅连接目标列表(env) {
	const 原始目标 = env.SUB_EDGE_IPS || env.SUB_NODE_ADDRESSES || env.SUB_NODE_ADDRESS || env.SUB_EDGE_IP || env.EDGEIP || env.PREFERRED_IP;
	if (!原始目标) return null;

	let 目标列表 = await 整理成数组(String(原始目标));
	const 原始黑名单 = env.BAD_EDGE_IPS || env.BAD_SUB_EDGE_IPS;
	if (原始黑名单) {
		const 黑名单 = new Set((await 整理成数组(String(原始黑名单))).map(item => item.trim().replace(/^https?:\/\//i, '').split('/')[0].split(':')[0]));
		目标列表 = 目标列表.filter(item => !黑名单.has(String(item).trim().replace(/^https?:\/\//i, '').split('/')[0].split(':')[0]));
	}

	const 去重目标 = [...new Set(目标列表.map(item => String(item).trim()).filter(Boolean))];
	return 去重目标.length === 0 ? null : 去重目标;
}
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	if (!('订阅连接目标' in config_JSON)) config_JSON.订阅连接目标 = null;
	const 环境订阅连接目标 = await 读取订阅连接目标列表(env);
	if (环境订阅连接目标) config_JSON.订阅连接目标 = 环境订阅连接目标;
	...
	const ECHLINK参数 = 生成ECHLINK参数(config_JSON, host);
	const { type: 传输协议, 路径字段名, 域名字段名 } = 获取传输协议配置(config_JSON);
	const ALPNLINK参数 = config_JSON.协议类型 !== 'ss' && 传输协议 === 'ws' ? '&alpn=h2%2Chttp%2F1.1' : '';
	...
	config_JSON.LINK = config_JSON.协议类型 === 'ss'
		...
		: `${config_JSON.协议类型}://${userID}@${host}:443?security=tls&type=${传输协议 + ECHLINK参数}&${域名字段名}=${host}&fp=${config_JSON.Fingerprint}${ALPNLINK参数}&sni=${host}&${路径字段名}=${encodeURIComponent(传输路径参数值) + TLS分片参数}&encryption=none#${encodeURIComponent(config_JSON.优选订阅生成.SUBNAME)}`;
